...
async function main() {
  ...
  for (const u of users) {
    ...
    await prisma.wallet.create({
      data: {
        userId: u.id,
        balance: isBuyer ? 100000000 : 5000000, // 100M VND for buyers, 5M for sellers
      },
    });
  }
  ...
  // Helper to create product, auction and optionally bids
  const createAuctionData = async (config: {
    ...
    views: number;
    bids?: Array<{ buyer: any; amount: number; time: Date }>;
  }) => {
    const product = await prisma.product.create({
      data: {
        ...
        status: config.status === AuctionStatus.ENDED ? ProductStatus.SOLD : ProductStatus.IN_AUCTION,
